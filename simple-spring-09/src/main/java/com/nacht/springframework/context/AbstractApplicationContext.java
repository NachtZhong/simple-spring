...
import com.nacht.springframework.bean.aware.ApplicationContextAwareProcessor;
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
    ...
    public void refresh() throws BeansException {
        ...
        /* 4. 添加ApplicationContextAwareProcessor, 这个BeanPostProcessor的作用是在其前置处理器中会将ApplicationContext实例传入到所有实现了ApplicationContextAware接口的bean中 */
        ...
        /* 5. 提前创建并注册BeanPostProcessor的bean到容器中, 且将这批bean存储在另外一个集合中(其他bean初始化的时候访问该集合来执行前置后置逻辑)*/
        registerBeanPostProcessors(beanFactory);

        /* 6. 提前实例化非lazy-init的单例对象 */
