...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader{
    ...
    private void doLoadBeanDefinition(InputStream inputStream) throws ClassNotFoundException {
        ...
        for(int i = 0 ; i < childNodes.getLength(); i++){
            ...
            String scope = childNode.getAttribute("scope");
            /*获取bean的class对象*/
            ...
            if(StrUtil.isNotEmpty(scope)){
                /*如果配置的不是有效的scope, 抛出异常*/
                if(!(ConfigurableBeanFactory.SCOPE_SINGLETON.equals(scope) || ConfigurableBeanFactory.SCOPE_PROTOTYPE.equals(scope))){
                    throw new BeansException("invalid scope config '" + scope + "' for bean " + beanName);
                }
